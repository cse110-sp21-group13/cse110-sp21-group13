...
describe('Basic user flow for monthly page', () => {
  ...
  it('Test 4: add a new bullet', async () => {
    const elementHandle = await page.$('#journal-frame');
    frame = await elementHandle.contentFrame();
  ...
  it('Test 5: check bullet in list', async () => {
    const elementHandle = await page.$('#journal-frame');
  ...
  it('test 7: refresh page', async () => {
    await page.reload({waitUntil: ['networkidle0', 'domcontentloaded']});
  }, 20000);

  it('Test 8: check number of bullets in list', async () => {
    const elementHandle = await page.$('#journal-frame');
    const frame = await elementHandle.contentFrame();
  ...
  it('test 9: add entry with type and signifier', async () => {
    const elementHandle = await page.$('#journal-frame');
    const frame = await elementHandle.contentFrame();
  ...
  it('Test 10: number of bullets should be 1', async () => {
    const elementHandle = await page.$('#journal-frame');
    const frame = await elementHandle.contentFrame();
    await frame.waitForSelector('li');
    const bullet = await frame.evaluate(() => {
  ...
  it('test 11: refresh page', async () => {
    await page.reload({waitUntil: ['networkidle0', 'domcontentloaded']});
  }, 20000);

  it('test 13: check bullet length', async () => {
    const elementHandle = await page.$('#journal-frame');
    const frame = await elementHandle.contentFrame();
    await frame.waitForSelector('ul');
  ...
  it('test 14: delete bullet', async () => {
    const elementHandle = await page.$('#journal-frame');
    const frame = await elementHandle.contentFrame();
